...
export default {
  ...
  methods: {
    ...
    async sendKeychain (amount, token) {
      ...
      const user = null
      const { success, msg, cancel, notInstalled, notActive } = await keychain(window, 'requestTransfer', user, this.sendHiveTo, parseFloat(amount).toFixed(3), this.invoice + ' v4v.app', token)
      if (success) {
    ...
    sendHivesigner (amount, token) {
      const dest = 'https://hivesigner.com/sign/transfer?to=' + this.sendHiveTo + '&from=&amount=' + amount + '%20' + token + '&memo=' + this.invoice + '%20v4v.app'
